  ...
  for i in "${debs[@]}"; do
    case $i in
      ...
      mina-create-legacy-genesis)
        ...
        source ./buildkite/scripts/cache/manager.sh read --root debs "$MINA_DEB_CODENAME/$i*" _build
      ;;
    esac
    source ./buildkite/scripts/cache/manager.sh read "debians/$MINA_DEB_CODENAME/${i}_*" $LOCAL_DEB_FOLDER
  done
fi
